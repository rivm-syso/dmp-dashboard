...
plot_rendered_compl <- reactiveVal()
...
renderPlotly({
  ...
  compl_freq <-  filtered_data() %>% 
                
                 count(., completion_cat, .drop = FALSE) 
  ...
  maxn <-  as.numeric(max(compl_freq$n) + 20) %>% 
  ...
  complggpl <- ggplot(compl_freq, 
                      mapping = aes(x = completion_cat, y = n)) +
               ...
               labs(x = NULL, 
                    y = 'Number of DMPs', 
                    title = 'DMP completion') +
              
               geom_text(aes(label = completion_cat), 
                         colour = "black", 
                         size = 2.8, 
                         nudge_y = 6) +
  ...
  plot_compl <- ggplotly(complggpl, source = 'completion_chart') %>% 
  ...
  session$onFlushed(\() plot_rendered_compl(TRUE), once = FALSE) 
