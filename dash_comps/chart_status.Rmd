...
plot_rendered_stat <- reactiveVal()
...
renderPlotly({
  ...
  status_freq <-  filtered_data() %>% 
                
                  count(., status, .drop = FALSE) 
  

  # Find the max number of DMPs, add 20, and round to the nearest 10. This value is used for
  # plotting the maximum Y value in the charts. 
  
  maxn <-  as.numeric(max(status_freq$n) + 20) %>% 
  ...
  #             plotly / ggplotly, hence geom_text. +
  # 
  # scale_y_continuous - set the breaks of the Y axis (here 0 to maxn with 10 increments), 
  #                      limit the Y axis (0 to maxn), and specify no padding of whitespace on the 
  #                      y axis by setting the expand to 0 
  
  statggpl <- ggplot(status_freq, 
                     mapping = aes(x = status, y = n)) +
              ...
              geom_text(aes(label = stringr::str_wrap(status, 5)), 
                        colour = "black", 
                        size = 2.8, 
                        nudge_y = 6) +
  ...
  plot_stat <- ggplotly(statggpl, source = 'status_chart') %>% 
  ...
  session$onFlushed(\() plot_rendered_stat(TRUE), once = FALSE)
